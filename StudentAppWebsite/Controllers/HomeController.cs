    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult NoteBookDetails(int? BookId = 0)
        {
            ...
            if (actionResult.IsSuccess)
            {
                ...
                model.Views = Convert.ToInt32(actionResult.dtResult.Rows[0]["Views"]);
                model.Likes = Convert.ToInt32(actionResult.dtResult.Rows[0]["Likes"]);

                double starRating = 0;
                if (model.Views > 0)
                {
                    double rating = (Convert.ToDouble(model.Likes * 5) / model.Views);
                    starRating = Convert.ToDouble(rating / 5) * 100;
                }
