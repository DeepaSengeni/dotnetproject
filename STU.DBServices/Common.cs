...
public class Common
{
    LogManager objLog = new LogManager();
    ...
    // static String ConnectionString = ConfigurationManager.ConnectionStrings["RoboConnectionString"].ToString();
    ...
    public static DataSet Execute_Command_Select()
    {
        ...
        for (int i = 0; i <= Commands.Length - 1; i++)
        {
            ...
            Commands[i].CommandType = CommandType.Text;
        ...
        {
            LogManager.InsertLog(LogTypeEnum.DatabaseOprationError, exx.Message, exx);
            return null;
        }
        finally
        {
    ...
    public static bool Execute_Command_CheckConnection()
    {
    ...
    public static bool Execute_Command_IUD(DataSet Result)
    {
        SqlConnection myConnection = new SqlConnection(ConnectionString);
        int Count = 0;
        myConnection.Open();
    ...
    //public DataTable Execute_Procedures_Select(string Proc_Name,string Text,int Country, int state,int city,string subject,int userID)
    //{
    //    SqlConnection myConnection = new SqlConnection(ConnectionString);
    //    DataTable dt = new DataTable();
    //    try
    ...
    public static DataTable Execute_Procedures_LoadData()
    {
        DataSet RetValue = new DataSet();
        SqlConnection myConnection = new SqlConnection(ConnectionString);
        SqlDataAdapter Adp = new SqlDataAdapter();
    ...
    public static string GetMimeType(string strFilePath)
    {

        string retval = ""; switch (strFilePath)
        {
