...
function AddStyle_ToTextbox() {
  $('#demo').find('input[type=text]').addClass('textbox');
...
function backspace() {
  ...
  str = $("#demo").html();
    if (c > 0 && c <= str.length) {
      $("#demo").focus().html(str.substring(0, c - 1) + str.substring(c, str.length));
        p.focus();
...
function GetData() {
  ...
  while ($("#demo").find('div').length > 0) {
    $("#demo").find('div').each(function () {
            var data = $(this).html();
